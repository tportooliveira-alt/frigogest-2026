  ...
  return (
    <div className="p-4 md:p-10 min-h-screen bg-[#f8fafc] technical-grid animate-reveal pb-20 font-sans">
      ...
      {activeTab === 'suppliers' && (() => {
        ...
        payables.filter(p => (p.categoria === 'COMPRA_GADO' || p.categoria === 'FRETE') && p.status !== 'ESTORNADO' && p.status !== 'CANCELADO').forEach(p => {
        ...
        transactions.filter(t => t.tipo === 'SAIDA' && (t.categoria === 'COMPRA_GADO' || t.categoria === 'FRETE') && !t.id.startsWith('TR-PAY-') && !t.id.startsWith('TR-ESTORNO-')).forEach(t => {
          let fornecedor = '';
          if (t.descricao) {
      ...
      {/* ABA FRETE - NOVA */}
      {activeTab === 'frete' && (() => {
          const fretePayables = payables.filter(p => p.categoria === 'FRETE' && p.status !== 'ESTORNADO' && p.status !== 'CANCELADO');
            const freteTx = transactions.filter(t => t.categoria === 'FRETE' && t.tipo === 'SAIDA' && !t.id.startsWith('TR-ESTORNO-'));
              const totalFreteAPagar = fretePayables.reduce((a, p) => a + (p.valor - (p.valor_pago || 0)), 0);
                const totalFretePago = freteTx.reduce((a, t) => a + t.valor, 0) + fretePayables.filter(p => p.status === 'PAGO').reduce((a, p) => a + p.valor, 0);
                  return (
                      <div className="max-w-7xl mx-auto space-y-8 animate-reveal">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="premium-card p-8 bg-blue-50 border-blue-100">
                                                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">Total Frete a Pagar</p>
                                                              <p className="text-3xl font-black text-blue-700">{formatCurrency(totalFreteAPagar)}</p>
                                                                      </div>
                                                                              <div className="premium-card p-8 bg-emerald-50 border-emerald-100">
                                                                                          <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">Total Frete Pago</p>
                                                                                                    <p className="text-3xl font-black text-emerald-700">{formatCurrency(totalFretePago)}</p>
                                                                                                            </div>
                                                                                                                    <div className="premium-card p-8 bg-slate-50">
                                                                                                                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Total Registros</p>
                                                                                                                                          <p className="text-3xl font-black text-slate-700">{fretePayables.length + freteTx.length}</p>
                                                                                                                                                  </div>
                                                                                                                                                        </div>
                                                                                                                                                              <div className="premium-card overflow-hidden shadow-xl">
                                                                                                                                                                        <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex items-center gap-3">
                                                                                                                                                                                  <Truck size={18} className="text-blue-600" />
                                                                                                                                                                                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fretes a Prazo — A Pagar</h4>
                                                                                                                                                                                                    </div>
                                                                                                                                                                                                            <div className="overflow-x-auto">
                                                                                                                                                                                                                      <table className="technical-table">
                                                                                                                                                                                                                                  <thead><tr><th>Vencimento</th><th>Lote</th><th>Transportadora</th><th className="text-right">Valor</th><th className="text-right">Pago</th><th className="text-center">Status</th><th className="text-center">Ação</th></tr></thead>
                                                                                                                                                                                                                                              <tbody>
                                                                                                                                                                                                                                                            {fretePayables.length === 0 && <tr><td colSpan={7} className="py-16 text-center text-xs text-slate-300 font-black uppercase tracking-widest">Nenhum frete a prazo registrado</td></tr>}
                                                                                                                                                                                                                                                                          {fretePayables.sort((a,b) => a.data_vencimento.localeCompare(b.data_vencimento)).map(p => {
                                                                                                                                                                                                                                                                                          const isLate = p.status !== 'PAGO' && p.data_vencimento < today;
                                                                                                                                                                                                                                                                                                          return (
                                                                                                                                                                                                                                                                                                                            <tr key={p.id} className={`hover:bg-slate-50 transition-colors group ${isLate ? 'bg-rose-50/10' : ''}`}>
                                                                                                                                                                                                                                                                                                                                                <td className={`text-[11px] font-extrabold ${isLate ? 'text-rose-600' : 'text-slate-400'}`}>{formatDateBR(p.data_vencimento)}</td>
                                                                                                                                                                                                                                                                                                                                                                    <td><span className="text-[10px] font-mono font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{p.id_lote || '-'}</span></td>
                                                                                                                                                                                                                                                                                                                                                                                        <td className="font-bold text-slate-800 uppercase text-xs">{p.beneficiario || p.descricao}</td>
                                                                                                                                                                                                                                                                                                                                                                                                            <td className="text-right font-black text-slate-900">{formatCurrency(p.valor)}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                <td className="text-right font-bold text-emerald-600">{formatCurrency(p.valor_pago || 0)}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                    <td className="text-center"><span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${p.status === 'PAGO' ? 'bg-emerald-50 text-emerald-600' : isLate ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'}`}>{p.status}</span></td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <td className="text-center">{p.status !== 'PAGO' && <button onClick={() => { setPayableToPay(p); setPayableAmountStr((p.valor - (p.valor_pago || 0)).toFixed(2).replace('.', ',')); }} className="w-9 h-9 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center mx-auto"><DollarSign size={16} /></button>}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          </tr>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          );
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        })}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    </tbody>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  <div className="premium-card overflow-hidden shadow-xl">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex items-center gap-3">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    <CheckCircle size={18} className="text-emerald-600" />
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fretes À Vista (Pagos no Ato)</h4>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              <div className="overflow-x-auto">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <table className="technical-table">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    <thead><tr><th>Data</th><th>Lote</th><th>Transportadora</th><th className="text-right">Valor</th></tr></thead>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <tbody>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              {freteTx.length === 0 && <tr><td colSpan={4} className="py-16 text-center text-xs text-slate-300 font-black uppercase tracking-widest">Nenhum frete à vista registrado</td></tr>}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            {freteTx.sort((a,b) => b.data.localeCompare(a.data)).map(t => (
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              <td className="text-[11px] font-extrabold text-slate-400">{formatDateBR(t.data)}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <td><span className="text-[10px] font-mono font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{t.referencia_id || '-'}</span></td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  <td className="font-bold text-slate-800 uppercase text-xs">{(t.descricao || '').split(' - ')[1] || t.descricao}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    <td className="text-right font-black text-slate-900">{formatCurrency(t.valor)}</td>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    </tr>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ))}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              </tbody>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </table>
