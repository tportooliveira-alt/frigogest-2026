...
const Expedition: React.FC<ExpeditionProps> = ({ stock, clients, batches, onConfirmSale, onBack, salesHistory }) => {
  ...
  const handleConfirm = () => {
    ...
    onConfirmSale({ client: selectedClient, items: itemsWithWeights, pricePerKg, extrasCost, pagoNoAto, prazoDias, metodoPagamento });
    setSelectedClient(null); setSelectedItems([]); setPricePerKg(0); setExtrasCost(0); setPagoNoAto(false); setPrazoDias(30); setMetodoPagamento('OUTROS');
