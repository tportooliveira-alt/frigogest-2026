...
const MarketingHub: React.FC<MarketingHubProps> = ({ data, onBack }) => {
    ...
    // Estados dos modais de Marketing
    const [activeModal, setActiveModal] = useState<null | 'marcas' | 'cartao' | 'catalogo' | 'etiquetas' | 'leads' | 'disparo'>('null' as any);
    const [leadsSearch, setLeadsSearch] = useState('');
    const [disparoMsg, setDisparoMsg] = useState('');
    const [disparoClients, setDisparoClients] = useState<string[]>([]);
    const [disparoSent, setDisparoSent] = useState(false);
    const [imagenError, setImagenError] = useState('');
    const IMAGEN_TEMPLATES = [
        { label: '🥩 Promocao Carne', prompt: 'Fotografia profissional de cortes nobres de carne bovina brasileira arrumados em tabua de madeira escura, iluminacao quente, apetitoso, alta qualidade' },
        { label: '📦 Kit Churrasco', prompt: 'Kit churrasco completo: cortes nobres de carne, carvao, chimichurri em caixa rustica, vista de cima, fotografia profissional' },
        { label: '📱 Banner WhatsApp', prompt: 'Banner horizontal limpo para distribuidora de carnes brasileira, cores vermelho e preto, visual premium, minimalista, profissional' },
        { label: '🏪 Post Instagram', prompt: 'Post quadrado para Instagram: bife suculento grelhado na frigideira de ferro com fumaca, fotografia de comida, cores vibrantes' },
        { label: '🎉 Natal/Festas', prompt: 'Promocao de Natal para empresa de carnes: cesta premium de carne bovina com fitas vermelhas, festivo, elegante, profissional' },
        { label: '📋 Catalogo Cortes', prompt: 'Catalogo profissional mostrando cortes bovinos brasileiros identificados: Picanha, Alcatra, Patinho, Acem, Costela, fundo limpo' },
    ];
    const generateImage = useCallback(async (prompt: string) => {
        const key = (import.meta as any).env.VITE_AI_API_KEY as string
            ...
            || '';
        if (!key) {
            setImagenError('⚠️ Configure VITE_AI_API_KEY no arquivo .env');
            return;
        }
        if (!prompt.trim()) { setImagenError('Digite uma descrição para a imagem'); return; }
        ...
        try {
            ...
            try {
                const r = await (ai.models as any).generateImages({
                    ...
                    prompt: `Marketing profissional distribuidora de carnes: ${prompt}. Alta qualidade, fotografia comercial.`,
                    config: { numberOfImages: 1, outputMimeType: 'image/jpeg' },
                });
                const b64 = r?.generatedImages?.[0]?.image?.imageBytes;
                if (b64) {
                    setGeneratedImages(prev => [{ url: `data:image/jpeg;base64,${b64}`, prompt, ts: new Date() }, ...prev].slice(0, 12));
                    found = true;
                }
            } catch {
                ...
                const r2 = await ai.models.generateContent({
                    ...
                    contents: { parts: [{ text: `Crie imagem profissional de marketing para distribuidora de carnes: ${prompt}.` }] },
                ...
                for (const p of parts) {
                    if ((p as any).inlineData) {
                        ...
                        setGeneratedImages(prev => [{ url: `data:${mimeType};base64,${b64}`, prompt, ts: new Date() }, ...prev].slice(0, 12));
                        found = true; break;
                    }
                }
            }
            if (!found) setImagenError('Modelo de imagem indisponível no momento. Tente novamente em alguns minutos.');
        } catch (e: any) {
            setImagenError(`Erro: ${e.message}`);
        } finally {
    ...
    const [designHistory, setDesignHistory] = useState<{ html: string; title: string; ts: Date }[]>([]);

    const DESIGN_TEMPLATES = [
        { label: '📱 Post Promo', desc: 'Post Instagram com precos', prompt: 'Crie um post para Instagram de promocao de carnes nobres da FrigoGest. Inclua: titulo PROMOCAO CARNES NOBRES, precos (Picanha R$79,90/kg, Alcatra R$49,90/kg, Costela R$39,90/kg), selo FRETE GRATIS acima R$300, logo FrigoGest, cores vermelho/preto, visual premium.' },
        { label: '📋 Cardapio', desc: 'Cardapio de cortes', prompt: 'Crie um cardapio elegante de cortes bovinos da FrigoGest. Categorias: Cortes Nobres (Picanha, Alcatra, Maminha), Cortes Populares (Acem, Patinho, Musculo), Kits Churrasco. Visual premium, fundo escuro, letras douradas.' },
        { label: '🎉 Convite Evento', desc: 'Convite para degustacao', prompt: 'Crie um convite elegante para evento de degustacao de carnes premium da FrigoGest. Data: Sabado 15h. Local: Sede FrigoGest. Inclua detalhes do menu, dress code casual chique, RSVP via WhatsApp. Visual sofisticado.' },
        { label: '📦 Kit Presente', desc: 'Banner kit presente', prompt: 'Crie um banner para kit presente de carnes nobres da FrigoGest. Kit Especial: 2kg Picanha + 1kg Alcatra + temperos. Preco: R$299. Embalagem premium com laco. Ideal para presentes corporativos. Visual luxo.' },
        { label: '📊 Infografico', desc: 'Cortes bovinos', prompt: 'Crie um infografico educativo mostrando os principais cortes bovinos brasileiros com suas caracteristicas: Picanha (macio, churrasco), Alcatra (versatil), Patinho (magro), Costela (sabor), Acem (cozidos). Visual limpo e informativo.' },
        { label: '🏷️ Etiqueta QR', desc: 'Etiqueta produto', prompt: 'Crie uma etiqueta de produto premium para carne bovina da FrigoGest. Inclua: nome do corte, peso, data de validade, selo SIF, espaco para QR code de rastreabilidade, logo FrigoGest. Visual clean e profissional.' },
    ];

    const generateDesign = useCallback(async (prompt: string) => {
        const key = (import.meta as any).env.VITE_AI_API_KEY as string || '';
        if (!key) { setDesignError('Chave Gemini nao configurada'); return; }
        if (!prompt.trim()) { setDesignError('Descreva o design que deseja'); return; }
        setDesignLoading(true); setDesignError('');
        try {
            const ai = new GoogleGenAI({ apiKey: key });
            const r = await ai.models.generateContent({
                model: 'gemini-2.0-flash-exp',
                contents: { parts: [{ text: `Voce e um designer grafico profissional. Crie um design em HTML+CSS inline completo e pronto para renderizar. O design deve ser visualmente impactante, moderno e premium. Use cores vibrantes, gradientes, sombras e tipografia forte. O HTML deve ser auto-contido (sem links externos). Dimensoes: 600x800px. IMPORTANTE: Retorne APENAS o codigo HTML, sem explicacao, sem markdown, sem blocos de codigo. Comece direto com <div> e termine com </div>.\n\nDesign solicitado: ${prompt}` }] }
            });
            ...
            if (html.includes('<div')) {
                setDesignHTML(html);
                setDesignHistory(prev => [{ html, title: prompt.slice(0, 50), ts: new Date() }, ...prev].slice(0, 8));
            } else {
                setDesignError('A IA nao conseguiu gerar o design. Tente novamente.');
            }
        } catch (e: any) { setDesignError(`Erro: ${e.message}`); } finally { setDesignLoading(false); }
    }, []);

    const downloadDesignAsImage = useCallback(() => {
        if (!designHTML) return;
        const blob = new Blob([`<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{margin:0;padding:0;}</style></head><body>${designHTML}</body></html>`], { type: 'text/html' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `design-frigogest-${Date.now()}.html`;
    ...
    const renderStitchConfig = () => (
        <div className="space-y-6 animate-fade-in relative z-10 w-full max-w-5xl mx-auto">
            {/* Header */}
            <div className="bg-gradient-to-r from-fuchsia-900/40 to-pink-900/40 border border-fuchsia-500/30 rounded-3xl p-8 backdrop-blur-xl mb-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-black text-fuchsia-400 uppercase tracking-wider mb-2 flex items-center gap-3">
                            <Paintbrush className="w-8 h-8" /> Studio de Design IA
                        </h2>
                        <p className="text-fuchsia-200/70 max-w-xl text-sm">
                            Crie posts, banners, cardapios e materiais de marketing profissionais em segundos com IA.
                        </p>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 rounded-full text-emerald-400 text-[9px] font-black uppercase tracking-widest">
                            <CheckCircle2 size={10} className="inline mr-1" /> Stitch Conectado
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                {DESIGN_TEMPLATES.map((t, i) => (
                    <button key={i} onClick={() => { setDesignPrompt(t.prompt); generateDesign(t.prompt); }}
                        className="bg-black/40 border border-white/10 rounded-2xl p-4 text-left hover:border-fuchsia-500/50 hover:bg-fuchsia-500/5 transition-all group">
                        <p className="text-lg mb-1">{t.label.split(' ')[0]}</p>
                        <p className="text-sm font-black text-white uppercase tracking-wider">{t.label.split(' ').slice(1).join(' ')}</p>
            ...
            <div className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                <h3 className="text-sm font-black text-white/50 uppercase tracking-widest mb-3">Descreva seu Design</h3>
                <div className="flex gap-3">
                    <textarea value={designPrompt} onChange={e => setDesignPrompt(e.target.value)}
                        placeholder="Ex: Crie um banner de promocao de picanha com preco R$79,90/kg, visual premium com fundo escuro..."
                        rows={3}
                        className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-fuchsia-500/50 resize-none" />
                    <button onClick={() => generateDesign(designPrompt)} disabled={designLoading}
                        className="px-6 bg-gradient-to-b from-fuchsia-500 to-fuchsia-700 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:shadow-lg hover:shadow-fuchsia-500/30 transition-all disabled:opacity-50 flex flex-col items-center justify-center gap-2 min-w-[100px]">
                        {designLoading ? <Loader2 size={20} className="animate-spin" /> : <Wand2 size={20} />}
                        {designLoading ? 'Criando...' : 'Criar Design'}
                    </button>
                </div>
                {designError && <div className="mt-3 px-4 py-2 bg-red-500/20 border border-red-500/30 rounded-xl text-red-400 text-xs font-bold">{designError}</div>}
            </div>
            ...
            {designHTML && (
                <div className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-black text-white/50 uppercase tracking-widest">Preview do Design</h3>
                        <div className="flex gap-2">
                            <button onClick={downloadDesignAsImage}
                                className="flex items-center gap-2 px-4 py-2 bg-emerald-500/20 border border-emerald-500/30 rounded-xl text-emerald-400 text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500/30 transition-all">
                                <Download size={14} /> Baixar HTML
                            </button>
                            <button onClick={() => generateDesign(designPrompt)}
                                className="flex items-center gap-2 px-4 py-2 bg-fuchsia-500/20 border border-fuchsia-500/30 rounded-xl text-fuchsia-400 text-[10px] font-black uppercase tracking-widest hover:bg-fuchsia-500/30 transition-all">
                                <RefreshCw size={14} /> Gerar Novo
                            </button>
                        </div>
                    </div>
                    <div className="bg-white rounded-xl overflow-hidden shadow-2xl mx-auto max-w-[600px]">
            ...
            {designHistory.length > 1 && (
                <div className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                    <h3 className="text-sm font-black text-white/50 uppercase tracking-widest mb-4">Historico de Designs ({designHistory.length})</h3>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {designHistory.map((d, i) => (
                            <button key={i} onClick={() => setDesignHTML(d.html)}
                                className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all cursor-pointer border-2 border-transparent hover:border-fuchsia-500">
                                <div className="h-32 overflow-hidden" style={{ transform: 'scale(0.3)', transformOrigin: 'top left', width: '333%', height: '106px' }}>
                                    <div dangerouslySetInnerHTML={{ __html: d.html }} />
                                </div>
                                <div className="p-2 bg-slate-50">
                                    <p className="text-[9px] font-bold text-slate-500 truncate">{d.title}</p>
                                </div>
                            </button>
                        ))}
                    </div>
            ...
            {/* Canais Conectados */}
            <div className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                <h3 className="text-sm font-black text-white/50 uppercase tracking-widest mb-4">Canais de Disparo Conectados</h3>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                    <div className="flex items-center gap-3 p-3 bg-white/5 border border-emerald-500/30 rounded-xl">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white/5 border border-rose-500/30 rounded-xl">
                        <div className="w-8 h-8 rounded-full bg-rose-500/20 flex items-center justify-center">
                            <Instagram size={16} className="text-rose-400" />
                        </div>
                        <div>
                            <p className="text-xs font-bold text-white">Instagram</p>
                            <p className="text-[10px] text-rose-400">Conectado</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-white/5 border border-blue-500/30 rounded-xl">
                        <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center">
                            <Megaphone size={16} className="text-blue-400" />
                        </div>
                        <div>
                            <p className="text-xs font-bold text-white">Meta Ads</p>
                            <p className="text-[10px] text-white/40">Clique para conectar</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="space-y-6 animate-fade-in relative z-10 w-full max-w-lg mx-auto bg-[#faf8f8] rounded-[2.5rem] p-6 text-slate-800 shadow-2xl border border-white/20">
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {/* Block 1 — Minhas Marcas */}
                    <div onClick={() => setActiveModal('marcas')} className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 hover:shadow-lg hover:border-slate-200 transition-all cursor-pointer relative overflow-hidden active:scale-95">
                        ...
                        <span className="absolute bottom-4 right-4 text-[9px] font-black text-emerald-500 uppercase">Tocar →</span>
                    </div>

                    {/* Block 2 — Cartão B2B */}
                    <div onClick={() => setActiveModal('cartao')} className="bg-[#0f172a] rounded-[2rem] p-6 shadow-xl shadow-slate-900/20 hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden active:scale-95">
                        ...
                        <span className="absolute bottom-4 right-4 text-[9px] font-black text-rose-400 uppercase">Tocar →</span>
                    </div>

                    {/* Block 3 — Catálogos */}
                    <div onClick={() => setActiveModal('catalogo')} className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 hover:shadow-lg hover:border-slate-200 transition-all cursor-pointer active:scale-95">
                        ...
                        <span className="block mt-2 text-[9px] font-black text-emerald-500 uppercase">Tocar →</span>
                    </div>

                    {/* Block 4 — Etiquetas */}
                    <div onClick={() => setActiveModal('etiquetas')} className="bg-rose-50/50 rounded-[2rem] p-6 shadow-sm border border-rose-100 hover:shadow-lg hover:border-rose-200 transition-all cursor-pointer active:scale-95">
            ...
            <div className="bg-[#e11d48] rounded-[2.5rem] p-8 shadow-2xl shadow-rose-500/30 relative overflow-hidden">
                ...
                <div className="space-y-3">
                    <button onClick={() => setActiveModal('leads')} className="w-full flex items-center justify-between bg-white/10 hover:bg-white/20 transition-colors rounded-3xl p-4 border border-white/20 active:scale-95">
                    ...
                    <button onClick={() => { setDisparoMsg(''); setDisparoClients([]); setDisparoSent(false); setActiveModal('disparo'); }} className="w-full flex items-center justify-between bg-white/10 hover:bg-white/20 transition-colors rounded-3xl p-4 border border-white/20 active:scale-95">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-[#0a1929] to-indigo-950 p-6 lg:p-12 relative overflow-hidden font-sans">
            ...
            {/* ═══ MODAIS FUNCIONAIS ═══ */}
            {activeModal && activeModal !== ('null' as any) && (
                <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-4" onClick={() => setActiveModal(null)}>
                    <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
                    <div className="relative bg-white rounded-3xl w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-6 border-b border-slate-100">
                            <h3 className="font-black text-slate-900 text-lg">
                                {activeModal === 'marcas' && '🏷️ Minhas Marcas — Portfólio de Cortes'}
                                {activeModal === 'cartao' && '💼 Cartão B2B Digital'}
                                {activeModal === 'catalogo' && '📋 Catálogo de Cortes'}
                                {activeModal === 'etiquetas' && '🏷️ Etiquetas & QR Code'}
                                {activeModal === 'leads' && '🔍 Busca de Leads B2B'}
                                {activeModal === 'disparo' && '📤 Disparo em Massa WhatsApp'}
                            </h3>
                            <button onClick={() => setActiveModal(null)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200">✕</button>
                        </div>
                        ...
                        <div className="p-6">
                            ...
                            {activeModal === 'marcas' && (
                                <div className="space-y-3">
                                    <p className="text-slate-500 text-sm">Portfólio de cortes da sua distribuidora — clique para copiar o texto de apresentação.</p>
                                    {['🥩 Picanha — Corte premium, sabor único. A estrela do churrasco.',
                                        '🥩 Alcatra — Maciez e versatilidade para qualquer prato.',
                                        '🥩 Contrafilé — Equilíbrio perfeito entre sabor e rendimento.',
                                        '🥩 Maminha — Suculência garantida. Ideal para churrascos grandes.',
                                        '🥩 Costela — O corte do slow cook. Sabor intenso e profundo.',
                                        '🦴 Acém — O favorito das panelas. Muito rendimento.',
                                    ].map((item, i) => (
                                        <button key={i} onClick={() => { navigator.clipboard.writeText(item); alert('✅ Copiado!'); }}
                                            className="w-full text-left px-4 py-3 bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 rounded-2xl text-sm font-bold text-slate-700 transition-all">
                                            {item} <span className="text-emerald-500 text-[10px]">📋 Copiar</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                            ...
                            {activeModal === 'cartao' && (
                                <div className="space-y-4">
                                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-4">DISTRIBUIDORA DE CARNES</p>
                                        <h2 className="text-2xl font-black text-white mb-1">FrigoGest</h2>
                                        <p className="text-slate-300 text-sm mb-4">Carnes de qualidade para atacado e varejo</p>
                                        <div className="pt-4 border-t border-slate-700 text-[11px] text-slate-400 space-y-1">
                                            <p>📦 Pedidos mínimos a partir de 20kg</p>
                                            <p>🚚 Entrega na região</p>
                                            <p>💳 Prazo de pagamento disponível</p>
                                        </div>
                                    </div>
                                    <button onClick={() => {
                                        const txt = 'FrigoGest — Distribuidora de Carnes\n📦 Pedidos a partir de 20kg\n🚚 Entregas na região\n💳 Prazo disponível\n📞 Entre em contato para fazer seu pedido!';
                                        navigator.clipboard.writeText(txt);
                                        alert('✅ Texto do Cartão B2B copiado! Cole no WhatsApp.');
                                    }} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl transition-all">
                                        📋 Copiar Texto do Cartão
                                    </button>
                                </div>
                            )}
                            ...
                            {activeModal === 'catalogo' && (
                                <div className="space-y-3">
                                    <p className="text-slate-500 text-sm">Toque em um corte para copiar descrição completa para WhatsApp ou Instagram.</p>
                                    {[
                                        { nome: 'Picanha', preco: 'R$ 68/kg', desc: '🥩 *PICANHA* — O nobre dos cortes brasileiros. Ideal para grelha e churrasco. Disponível hoje! Peça já.' },
                                        { nome: 'Alcatra', preco: 'R$ 42/kg', desc: '🥩 *ALCATRA* — Versatilidade e sabor em cada pedaço. Perfeita para bifes e assados. Entregamos!' },
                                        { nome: 'Costela', preco: 'R$ 28/kg', desc: '🦴 *COSTELA* — Sabor intenso e inconfundível. O preferido do slow cook. Encomende agora!' },
                                        { nome: 'Maminha', preco: 'R$ 38/kg', desc: '🥩 *MAMINHA* — Suculência e maciez garantidas. Para churrasco que impressiona. Reserve a sua!' },
                                    ].map((c, i) => (
                                        <div key={i} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                                            <div>
                                                ...
                                                <p className="text-xs text-emerald-600 font-bold">{c.preco}</p>
                                            </div>
                                            <button onClick={() => { navigator.clipboard.writeText(c.desc); alert('✅ Copiado para WhatsApp!'); }}
                                                className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white text-[10px] font-black rounded-xl transition-all">
                                                📋 Copiar
                                            </button>
                                        </div>
                                    ))}
                            ...
                            {activeModal === 'etiquetas' && (
                                <div className="space-y-4 text-center">
                                    <p className="text-slate-500 text-sm">QR Code para clientes acessarem o catálogo ou enviarem pedidos pelo WhatsApp.</p>
                                    <div className="bg-slate-50 rounded-2xl p-6 flex flex-col items-center gap-4">
                                        ...
                                        <p className="text-xs text-slate-400 font-bold">QR Code gerado com o número de WhatsApp do sistema</p>
                                    </div>
                                    <button onClick={() => {
                                        const phone = data.clients[0]?.whatsapp || '5511999999999';
                                        const msg = encodeURIComponent('Olá! Vi o QR code e quero fazer um pedido. Podem me atender?');
                                        window.open(`https://wa.me/${phone}?text=${msg}`, '_blank');
                                    }} className="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl transition-all">
                                        📱 Abrir WhatsApp de Testes
                                    </button>
                                </div>
                            )}
                            ...
                            {activeModal === 'leads' && (
                                <div className="space-y-4">
                                    <p className="text-slate-500 text-sm">Pesquise clientes inativos ou identifique segmentos para prospecção.</p>
                                    <input value={leadsSearch} onChange={e => setLeadsSearch(e.target.value)}
                                        placeholder="Buscar por nome, segmento (ex: açougue, restaurante)..."
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:outline-none focus:border-indigo-400" />
                                    <div className="space-y-2 max-h-48 overflow-y-auto">
                                        {data.clients.filter(c =>
                                            !leadsSearch || c.nome_social.toLowerCase().includes(leadsSearch.toLowerCase())
                                        ).slice(0, 10).map((c, i) => (
                                            <div key={i} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl">
                                                <div>
                                                    <p className="font-black text-slate-900 text-sm">{c.nome_social}</p>
                                                    <p className="text-[10px] text-slate-400">{c.whatsapp || 'Sem WhatsApp'}</p>
                                                </div>
                                                {c.whatsapp && (
                                                    <button onClick={() => {
                                                        const msg = encodeURIComponent(`Olá ${c.nome_social}! Temos uma oferta especial de carnes para você. Quer ver nosso catálogo?`);
                                                        window.open(`https://wa.me/${c.whatsapp.replace(/\D/g, '')}?text=${msg}`, '_blank');
                                                    }} className="px-3 py-1 bg-green-600 hover:bg-green-500 text-white text-[10px] font-black rounded-xl transition-all">
                                                        📲 Contatar
                                                    </button>
                                                )}
                                            </div>
                                        ))}
                                        {data.clients.length === 0 && <p className="text-center text-slate-400 text-sm py-4">Nenhum cliente cadastrado ainda.</p>}
                                    </div>
                                </div>
                            )}
                            ...
                            {activeModal === 'disparo' && (
                                <div className="space-y-4">
                                    {!disparoSent ? (
                                        <>
                                            <p className="text-slate-500 text-sm">Selecione os clientes e escreva a mensagem. Cada link abrirá o WhatsApp individualmente.</p>
                                            <textarea value={disparoMsg} onChange={e => setDisparoMsg(e.target.value)}
                                                placeholder="Ex: Olá {nome}! Temos uma promoção especial de Picanha hoje. Quer aproveitar?"
                                                rows={4} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm resize-none focus:outline-none focus:border-indigo-400" />
                                            <div className="max-h-40 overflow-y-auto space-y-1">
                                                {data.clients.filter(c => c.whatsapp).map((c, i) => (
                                                    <label key={i} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-xl cursor-pointer">
                                                        <input type="checkbox" checked={disparoClients.includes(c.id_ferro)}
                                                            onChange={e => setDisparoClients(prev => e.target.checked ? [...prev, c.id_ferro] : prev.filter(x => x !== c.id_ferro))}
                                                            className="w-4 h-4 rounded" />
                                                        <span className="text-sm font-bold text-slate-700">{c.nome_social}</span>
                                                        <span className="text-[10px] text-slate-400">{c.whatsapp}</span>
                                                    </label>
                                                ))}
                                            </div>
                                            <button
                                                disabled={disparoClients.length === 0 || !disparoMsg.trim()}
                                                onClick={() => {
                                                    disparoClients.forEach(id => {
                                                        const client = data.clients.find(c => c.id_ferro === id);
                                                        if (client?.whatsapp) {
                                                            const personalMsg = disparoMsg.replace('{nome}', client.nome_social.split(' ')[0]);
                                                            setTimeout(() => window.open(`https://wa.me/${client.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(personalMsg)}`, '_blank'), 500);
                                                        }
                                                    });
                                                    setDisparoSent(true);
                                                }}
                                                className="w-full py-3 bg-green-600 hover:bg-green-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black rounded-2xl transition-all">
                                                📤 Disparar para {disparoClients.length} cliente(s)
                                            </button>
                                        </>
                                    ) : (
                                        <div className="text-center py-8">
                                            <CheckCircle2 size={48} className="text-emerald-500 mx-auto mb-4" />
                                            <h4 className="font-black text-slate-900 text-lg mb-2">Disparos Enviados!</h4>
                                            <p className="text-slate-500 text-sm">Links do WhatsApp foram abertos para {disparoClients.length} cliente(s).</p>
                                            <button onClick={() => { setDisparoSent(false); setActiveModal(null); }} className="mt-4 px-6 py-2 bg-slate-900 text-white font-black rounded-xl text-sm">Fechar</button>
                                        </div>
                                    )}
                                </div>
                            )}
            ...
            <div className="max-w-7xl mx-auto flex flex-col items-center">
                ...
                <div className="text-center space-y-4 mb-12 relative z-10 w-full animate-fade-in-down">
                    {/* BOTÃO VOLTAR */}
                    {onBack && (
                        <button onClick={onBack} className="absolute left-0 top-0 flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white/70 hover:text-white rounded-2xl border border-white/10 text-xs font-bold uppercase tracking-widest transition-all">
