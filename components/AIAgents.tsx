...
${suppliers.slice(0, 5).map(f => `- ${f.nome_fantasia} | ${f.cidade || 'N/A'} | Contato: ${f.telefone || 'N/A'}`).join('\n') || '- Sem fornecedores cadastrados'}
...
${clients.slice(0, 5).map(c => `- ${c.nome_social} | Perfil: ${c.perfil_compra || 'N/A'} | Cidade: ${c.cidade || 'N/A'}`).join('\n') || '- Cadastrar perfil de clientes para segmentação'}
...
${clients.filter(c => c.status !== 'INATIVO').slice(0, 10).map(c =>
                    `- ${c.nome_social} | Contato: ${c.whatsapp || 'N/A'} | Cidade: ${c.cidade || 'N/A'}`
